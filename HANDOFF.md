...
- **Репозиторий уже настроен на `sashik08/openclaw-stack`** в `install.sh` (REPO/RAW) и
  ...
  там же или через env `OPENCLAW_STACK_REPO`/`OPENCLAW_STACK_RAW`. Приватный репо создан на
  GitHub (аккаунт sashik08); пуш идёт по HTTPS-токену из `~/.git-credentials`.
...
## На чём остановились
Функционально проект завершён; код/тесты/CI/Docker/доки на месте, выгружен в приватный репо
`sashik08/openclaw-stack`. Не сделано (осознанно, на стороне мейнтейнера перед публикацией):
