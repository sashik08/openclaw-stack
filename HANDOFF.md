...
Дата: 2026-07-15 | Статус: завершён (готов к публикации репозитория)
...
- **Проект — НЕ git-репозиторий.** `git log/diff` недоступны; истории коммитов нет.
...
- **Плейсхолдеры `your-org/openclaw-stack`** в `install.sh` (REPO/RAW) и
  `internal/config/config.go` (`Defaults().RepoRawURL`) — ОБЯЗАТЕЛЬНО заменить перед публикацией,
  иначе install.sh ничего не скачает. См. `DEPLOYMENT.md` часть A1.
...
## На чём остановились
Функционально проект завершён; код/тесты/CI/Docker/доки на месте. Не сделано (осознанно, на
стороне мейнтейнера перед публикацией):
- Заменить плейсхолдеры репозитория (A1 в DEPLOYMENT.md).
- Сверить/зафиксировать теги образов апстрима.
- Выпустить первый релиз тегом `v*`, чтобы `install.sh` качал готовые бинари (иначе фолбэк —
  сборка из исходников).
- Не запускался реальный end-to-end на живой Ubuntu/Debian/CentOS с Docker — только сборка,
  vet, юнит-тесты и smoke-тесты CLI. Первый боевой прогон стоит сделать на чистой VM.
